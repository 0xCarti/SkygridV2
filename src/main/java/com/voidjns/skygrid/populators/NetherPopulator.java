...
public class NetherPopulator extends BlockPopulator {
    ...
    public NetherPopulator(FileConfiguration config){
        for(String key : config.getConfigurationSection("nether-mob-spawners").getKeys(false)) {
            try{
                double prob = config.getDouble("nether-mob-spawners." + key) * 100;
                for(int i = 0; i < prob; i++){
                    spawnerTypes.add(EntityType.valueOf(key));
                }
            }catch (IllegalArgumentException e) {
                System.out.println("[Nether Populator] " + key + " is not a valid entity!");
            }
        }

        if(spawnerTypes.size() > 10000){
            System.out.println("[Nether Populator] Allocated " + (spawnerTypes.size() - 10000)/100 + " extra percentage. Some blocks won't be spawned.");
            spawnerTypes = spawnerTypes.subList(0, 10000);
        }else if (spawnerTypes.size() < 10000){
            System.out.println("[Nether Populator] Allocated " + (10000 - spawnerTypes.size())/100 + " percentage less. Blazes will be used.");
            for(int i = 0; i < (10000-spawnerTypes.size()); i++){
                spawnerTypes.add(EntityType.BLAZE);
            }
        }
    }
